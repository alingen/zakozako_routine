...
/// 新規作成時に表示する、カスタム入力とプリセットの選択画面。
struct RoutinePresetSelectionView: View {
    let presets: [RoutinePreset]
    let onSelectCustom: () -> Void
    let onSelectPreset: (RoutinePreset) -> Void

    @State private var searchText = ""

    init(
        presets: [RoutinePreset] = RoutinePreset.all,
        onSelectCustom: @escaping () -> Void,
        onSelectPreset: @escaping (RoutinePreset) -> Void
    ) {
    ...
    private var filteredPresets: [RoutinePreset] {
